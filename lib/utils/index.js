...
/** @type {string} */
if (utils.isWin) {
  utils.npmExecutable = 'npm.cmd'
}
else if (isRoot()) {
  utils.npmExecutable = 'npm'
}
else {
  utils.npmExecutable = 'sudo npm'
}
...
utils.argv = (opts) => {
  const argv = yargs.options({
    ...
    update: { type: 'boolean', alias: ['update', 'U'] },
...
utils.getDependencyPackageJson = (...deps) => deps.map(dep => {
  ...
  let json
  try {
	json = require(Path.join(path, 'package.json'));
  }
  catch (e) {
    try {
		console.log(`Try to link ${Path.basename(path)}`)
		console.log(`${dep}`)
		utils.npm(`link ${dep}`);
		json = require(Path.join(path, 'package.json'));
	}
		...
		try {
			console.log(`Try to install global ${Path.basename(path)}`)
			utils.npm(`install -g ${dep}`);
			utils.npm(`link ${dep}`);
			json = require(Path.join(path, 'package.json'));
		}
		catch (e) {
			console.log('\n==========================================================')
			console.error('[ERROR] ' + Path.basename(path) + ' is not found. Maybe the package is not installed yet.')
			console.error('[PACAKGE PATH] ' + Path.join(path, 'package.json'))	
			console.log('==========================================================')
 ...
 * @returns {Array<string>} paths
 */
utils.removeDependencyPackage = (...deps) => deps.map(dep => {
  let packagePath = utils.getDependencyPath(dep)
  //console.log(packagePath)
  fs.unlinkSync(packagePath[0])
});
